<!-- this page is the debrief that collects post survey questions;
    ...
    <script>
       ...
       let answer = '';
        ...
        let Main_question = ['1.   Who is the biggest enemy of Kung Fury?', '1. What happened to the electronic bald eagle supported the dictator?'];

let Answer_a = ['A) The police department', 'A) It defeated the heroes and the dictator still runs the country.'];

let Answer_b = ['B) Barbariana', 'B) The dictator shut the bald eagle down.'];
let Answer_c = ['C) An electronic computer', 'C) The bald eagle lost power during the fight.']
let Answer_d = ['D) Thor', 'D) The heroes were able to crush the eagle, but it may still be alive.'];
let Answer_e =['E) Adolf Hitler', 'E) The dictator fought against the bald eagle.'];
